#include "lakeformation/lf_credentials.hpp"
...
LakeFormationClientConfig BuildLakeFormationClientConfig(ClientContext &context, IcebergCatalog &catalog) {
	...
	// Resolve the catalog's storage secret directly through DuckDB's SecretManager. We deliberately do
	// not call IcebergCatalog::GetStorageSecret: this extension is a separate loadable binary and links
	// only against DuckDB core, not against the iceberg extension's out-of-line symbols.
	auto transaction = CatalogTransaction::GetSystemCatalogTransaction(context);
	auto secret_entry = context.db->GetSecretManager().GetSecretByName(transaction, sigv4.secret);
	if (!secret_entry) {
		throw InvalidConfigurationException(
		    "Lake Formation integration requires a storage secret ('%s') on the Iceberg catalog", sigv4.secret);
	}
