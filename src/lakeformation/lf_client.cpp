...
static string GetDatabaseName(IcebergSchemaEntry &schema) {
	// Glue databases are single-level namespaces. We read the public namespace_items directly rather
	// than calling iceberg's IRCPathComponent helper (an out-of-line symbol unavailable to this
	// separately-linked extension). Nested namespaces are joined with '.'; special characters are not
	// URL-encoded here, which is sufficient for the common Glue case.
	return StringUtil::Join(schema.namespace_items, ".");
}
...
LakeFormationTablePolicy LakeFormationClient::FetchTablePolicy(IcebergSchemaEntry &schema, const string &table_name,
	...
	// Schema-dependent validation (column/cell permissions) is deferred to filter application time,
	// where iceberg hands us the bound table schema.
